      ...
      >
        <div className="absolute -top-6 left-1/2 transform -translate-x-1/2">
          <div className="relative">
            <div className="absolute inset-0 blur-xl bg-yassin-neon-purple/30 rounded-full"></div>
            <div className="relative z-10 bg-gradient-to-r from-yassin-neon-purple to-yassin-neon-blue p-3 rounded-full animate-pulse-glow">
        ...
        <Accordion type="single" collapsible className="w-full divide-y divide-white/10">
          {faqItems.map((item, index) => (
            ...
            >
              <AccordionTrigger className="text-white text-lg font-medium hover:text-yassin-neon-purple hover:no-underline transition-colors group">
                <span className="group-hover:translate-x-1 transition-transform duration-300">{item.question}</span>
              </AccordionTrigger>
              <AccordionContent className="text-white/80">
                <div className="pl-1 border-l-2 border-yassin-neon-purple/50">{item.answer}</div>
              </AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
